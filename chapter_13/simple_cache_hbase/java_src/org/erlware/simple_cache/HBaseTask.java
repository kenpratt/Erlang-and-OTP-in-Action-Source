...
public class HBaseTask implements Runnable {
	 ...
	 * 
	 * @param mbox
	 *            The erlang mailbox to use for sending
	 * @param conn
	 *            The connection object to our hbase system
	 * @param from
	 *            The pid that the request came from
	 * @param action
	 *            The action that was requested (get/put)
	 * @param key
	 *            The key of the request
	 * @param value
	 *            The value if it exists (may be null if get is the action)
	 */
	public HBaseTask(OtpMbox mbox, HBaseConnector conn, OtpErlangPid from,
			OtpErlangRef ref, String action, String key, byte[] value) {
		super();
	...
	/**
	 * Get a value from the database and send it back to the requestor in the
	 * form
	 * 
	 * {get_result, Ref, <data>}
	 * 
	...
	private void doPut() throws IOException {
		...
		OtpErlangTuple res = new OtpErlangTuple(
				new OtpErlangObject[] { new OtpErlangAtom("put_result"), _ref,
						new OtpErlangAtom("ok") });
	...
	/**
	 * The run task of the runnable interface. Actually kicks off and does the
	 * work.
	 * 
	 */
	@Override
